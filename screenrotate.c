...
void rotate_screen(){
	...
	system(command);
	sprintf(command, "xinput set-prop \"%s\" \"Coordinate Transformation Matrix\" %s", "Wacom Pen and multitouch sensor Pen Pen (0x9d4c0b6a)", COOR[current_state]);
	system(command);
}
...
#endif
#if DEBUG
		fprintf(stderr, "accel_x: %10.4f\n", accel_x);
		fprintf(stderr, "accel_y: %10.4f\n", accel_y);
#endif
		if(rotation_changed())
			rotate_screen();
		sleep(2);
